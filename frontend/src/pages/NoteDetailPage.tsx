...
export default function NoteDetailPage() {
  ...
  const submitComment = async () => {
    if (!nickname.trim() || !commentText.trim() || !id) return
    setSubmitting(true)
  ...
  if (!note) return <div className="text-slate-400 py-10 text-center">加载中...</div>

  return (
    <div>
      <h1 className="text-3xl font-bold text-slate-900 mb-2">{note.title}</h1>
      <p className="text-sm text-slate-400 mb-8 font-mono">{note.createdAt?.slice(0, 10)}</p>
            ...
            <div key={c.id} className="flex gap-3">
              ...
              <div>
                <div className="flex items-baseline gap-2 mb-1">
                  <span className="text-sm font-medium text-slate-800">{c.nickname}</span>
